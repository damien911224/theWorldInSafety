import sys
sys.path.insert(0, "/home/damien/temporal-segment-networks/lib/caffe-action/python")
import caffe
import os
import glob
import sys
...
def build_net():
    ...
    spatial_net_weights = "/home/damien/temporal-segment-networks/models/twis_caffemodels/v1/twis_spatial_net_v1.caffemodel"
    temporal_net_proto = "/home/damien/temporal-segment-networks/models/twis/tsn_bn_inception_flow_deploy.prototxt"
    temporal_net_weights = "/home/damien/temporal-segment-networks/models/twis_caffemodels/v1/twis_temporal_net_v1.caffemodel"
...
def preProcessInputDataForTSN(vid_list, extract_dst_path, clip_dst_path, frame_src_path, frame_dst_path, flow_type = 'optical_flow', num_worker=8, num_worker_using_gpu=4):
    global len_video_list
    global len_scan_list
    global global_start_time
...
if __name__ == '__main__':
    video_src_path = "/media/damien/DATA/cvData/TWIS/v2"
    extract_dst_path = "/media/damien/DATA/cvData/TSN_data/TWIS/v2"
    clip_dst_path = "/media/damien/DATA2/cvData/TWIS/Clips_v2"
    frame_src_path = "/media/damien/DATA/cvData/TSN_data/TWIS/v2"
    frame_dst_path = "/media/damien/DATA2/cvData/TSN_data/TWIS/v2"
    split_path = "/home/damien/temporal-segment-networks/data/twis_splits"

    num_worker = 12
    num_worker_using_gpu = 8
    num_of_splits = 3
    flow_type = 'optical_flow'

    vid_list = []
    for ii in xrange(num_of_splits):
        train_split_fp = open(os.path.join(split_path,"trainlist0{}.txt".format(ii+1)), "r")
        test_split_fp = open(os.path.join(split_path, "testlist0{}.txt".format(ii+1)), "r")

        while True:
            train_line = train_split_fp.readline()
            if not train_line:
                break
            train_video_path = os.path.join(video_src_path, train_line.split(' ')[-2])
            if train_video_path not in vid_list:
                vid_list.append(train_video_path)

        while True:
            test_line = test_split_fp.readline()
            if not test_line:
                break
            test_video_path = os.path.join(video_src_path, test_line[:-1])
            if test_video_path not in vid_list:
                vid_list.append(test_video_path)

    print "Extracting Optical Flow: Number of Videos is {}".format(len(vid_list))
    print ""

    preProcessInputDataForTSN(vid_list, extract_dst_path, clip_dst_path, frame_src_path, frame_dst_path,
                              flow_type , num_worker, num_worker_using_gpu)

    sys.path.remove("/home/damien/temporal-segment-networks/lib/caffe-action/python")