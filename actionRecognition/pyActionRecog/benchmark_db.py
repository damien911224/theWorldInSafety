...
def parse_directory(path, rgb_prefix='img_', flow_x_prefix='flow_x_', flow_y_prefix='flow_y_'):
    ...
    def count_files(directory, prefix_list):
        rgb_count = len(glob.glob(directory + '/images/' + prefix_list[0] + '*'))
        flow_x_count = len(glob.glob(directory + '/optical_flow/' + prefix_list[1] + '*'))
        flow_y_count = len(glob.glob(directory + '/optical_flow/' + prefix_list[2] + '*'))
        cnt_list = [rgb_count, flow_x_count, flow_y_count]
        return cnt_list
...
def parse_ucf_splits():
    class_ind = [x.strip().split() for x in open('/home/damien/temporal-segment-networks/data/ucf101_splits/classInd.txt')]
    ...
    splits = []
    for i in xrange(1, 4):
        train_list = [line2rec(x) for x in open('/home/damien/temporal-segment-networks/data/ucf101_splits/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('/home/damien/temporal-segment-networks/data/ucf101_splits/testlist{:02d}.txt'.format(i))]
        splits.append((train_list, test_list))
    return splits

## Dataset specific split file parse
def parse_twis_splits():
    class_ind = [x.strip().split() for x in open('../data/twis_splits/classInd.txt')]
    class_mapping = {x[1]:int(x[0])-1 for x in class_ind}
    ...
    splits = []
    for i in xrange(1, 4):
        train_list = [line2rec(x) for x in open('../data/twis_splits/trainlist{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open('../data/twis_splits/testlist{:02d}.txt'.format(i))]
        splits.append((train_list, test_list))
    return splits

def parse_hmdb51_splits():
    # load split file
    class_files = glob.glob('/home/damien/temporal-segment-networks/data/hmdb51_splits/*split*.txt')

    # load class list
    class_list = [x.strip() for x in open('/home/damien/temporal-segment-networks/data/hmdb51_splits/class_list.txt')]
